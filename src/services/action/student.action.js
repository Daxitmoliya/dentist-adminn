 ...
 export const updateAdminSuccess = (adminId, updatedData) => ({
  type: UPDATE_ADMIN,
  payload: { id: adminId, data: updatedData },
});
  ...
  export const updateAdmin = (adminId, newdata) => {
    return (dispatch) => {
      ...
      const updatedDoc = doc(db, "servicies", adminId);
  
      updateDoc(updatedDoc, newdata)
        .then(() => {
          console.log('Update successful');
          dispatch(updateAdminSuccess(adminId, newdata));  
          dispatch(fetchData()); 
        })
        .catch((error) => {
